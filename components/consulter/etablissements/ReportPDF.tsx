    ...
    const loadPDF = async () => {
      try {
        ...
        const ReportPDF = ({ mainItem, hotelItems, subItems, generatedAt }: any) => {
          const total = mainItem?.nbetablissements || 0;
            ...
            <Document>
              <Page size="A4" style={pdfStyles.page}>
                ...
                <View style={pdfStyles.header}>
                  <Text style={pdfStyles.title}>Rapport Analytique des Établissements</Text>
                ...
                <View style={pdfStyles.section}>
                  ...
                  <View style={pdfStyles.card}>
                    <View style={pdfStyles.cardRow}>
                      <Text style={pdfStyles.cardLabel}>Total des établissements</Text>
                      <Text style={pdfStyles.cardValue}>{total.toLocaleString('fr-FR')}</Text>
                    </View>
                    {hotelItems.slice(0, 3).map((item: MenuItem, index: number) => (
          ...
          return (
            ...
            >
              {({ loading }: { loading: boolean }) => (
                <>
                  <FileText size={20} />
                  <span>{loading ? "Préparation du PDF..." : "Générer un rapport analytique par IA"}</span>
                  <Download size={18} />
                </>
              )}
            </PDFDownloadLink>
