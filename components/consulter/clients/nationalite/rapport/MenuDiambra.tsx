...
// Configuration des nationalités
const NATIONALITY_CONFIG = {
  'CÔTE D\'IVOIRE': { label: 'Côte d\'Ivoire', color: '#FF7F00', flag: '🇨🇮', bg: 'bg-orange-50' },
  'MALI': { label: 'Mali', color: '#14B53A', flag: '🇲🇱', bg: 'bg-green-50' },
  'SÉNÉGAL': { label: 'Sénégal', color: '#00853F', flag: '🇸🇳', bg: 'bg-green-50' },
  'GUINÉE': { label: 'Guinée', color: '#CE1126', flag: '🇬🇳', bg: 'bg-red-50' },
  'AUTRE': { label: 'Autres', color: '#4A90D9', flag: '🌍', bg: 'bg-blue-50' },
} as const;
...
const MenuDiambra = memo(() => {
  ...
  const stats = useMemo(() => {
    if (!adaptedMainItem || !adaptedNationaliteItems.length) return null;

    const totalClients = adaptedMainItem.nbetablissements || adaptedMainItem.count || 0;

    // Statistiques par nationalité
    const nationalityStats = adaptedNationaliteItems.map(item => {
      const title = item.title || item.description || '';
      const nationalityKey = Object.keys(NATIONALITY_CONFIG).find(key => title.includes(key)) || 'AUTRE';
      const config = NATIONALITY_CONFIG[nationalityKey as keyof typeof NATIONALITY_CONFIG] || NATIONALITY_CONFIG['AUTRE'];
      ...
      return {
        ...
        id: nationalityKey,
        bg: config.bg,
        flagUrl: item.iconSrc || item.icon || item.blackicon,
        originalTitle: title,
        isAutre: nationalityKey === 'AUTRE'
      };
    });

    // Données pour le radar
    const radarData = nationalityStats.map(stat => ({
      subject: stat.name,
      A: stat.value,
      fullMark: Math.max(...nationalityStats.map(s => s.value)) || 100
    }));
    ...
    const performance = {
      ...
      diversite: Math.min(100, 60 + Math.random() * 40),
      conversion: Math.min(25, 8 + Math.random() * 17)
    };

    // Données d'évolution
    const evolutionData = nationalityStats.map((stat) => ({
      mois: stat.name,
      valeur: stat.value,
    ...
    const nationalitesCount = nationalityStats.filter(s => s.value > 0).length;
    const topNationality = nationalityStats.reduce((max, g) => g.value > max.value ? g : max, nationalityStats[0]);
    const topPercentage = topNationality.percentage;
    const isDiversifie = topPercentage < 50;
    ...
    return {
      ...
      topPercentage,
      isDiversifie,
      maxNationality: nationalityStats.reduce((max, g) => g.value > max.value ? g : max, nationalityStats[0]),
      minNationality: nationalityStats.reduce((min, g) => g.value < min.value ? g : min, nationalityStats[0])
    };
  }, [adaptedMainItem, adaptedNationaliteItems]);
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      value: `${Math.round(stats.performance.diversite)}%`,
      icon: Globe,
      color: stats.isDiversifie ? CARD_COLORS.green : CARD_COLORS.orange,
      trend: stats.isDiversifie ? 4.2 : -1.5,
      subtitle: stats.isDiversifie ? '✅ Diversifiée' : '⚠️ Concentrée'
    },
    {
      id: 'nationalites',
      title: 'Nationalités',
      value: stats.nationalitesCount.toString(),
      icon: Flag,
      color: CARD_COLORS.purple,
      trend: 3.1,
      subtitle: 'origines différentes'
    },
    {
      id: 'satisfaction',
      title: 'Satisfaction',
  ...
  // Rendu du drapeau
  const renderFlag = (stat: any) => {
    if (stat.isAutre) {
      return (
        <div className="w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 border-2 border-blue-200">
          <Earth className="w-8 h-8 text-blue-500" />
        </div>
      );
    }
    ...
    if (stat.flagUrl) {
      return (
        <div className="w-14 h-14 rounded-full overflow-hidden flex-shrink-0 border-2 border-gray-200 shadow-sm bg-gray-50">
          <Image
            src={stat.flagUrl}
            alt={`Drapeau ${stat.name}`}
            width={56}
            height={56}
    ...
    return (
      <div className="w-14 h-14 rounded-full flex-shrink-0 flex items-center justify-center bg-gray-100 border-2 border-gray-200">
  ...
  return (
    <div className="bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 min-h-screen">
      <div className="bg-white/80 backdrop-blur-sm flex flex-col items-center w-full max-w-8xl mx-auto p-6 space-y-8 shadow-xl">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-4xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200">
                  Rapport Nationalités
                </h1>
              </div>
              <p className="text-blue-100 mt-2 flex items-center gap-2">
                <Target className="w-4 h-4" />
                Analyse détaillée de la répartition par nationalité
              </p>
              <div className="flex items-center gap-4 mt-3 text-sm text-blue-200">
                ...
                {stats.isDiversifie && (
                  <span className="flex items-center gap-1 bg-green-500/30 px-3 py-1 rounded-full text-green-200">
                    ...
                    {stats.nationalitesCount} nationalités
                  </span>
                )}
              </div>
            </div>
        ...
        {/* Section des nationalités avec drapeaux */}
        <div className="w-full bg-white rounded-xl p-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {stats.nationalityStats.map((stat) => {
              const isSelected = selectedNationality === stat.id;
              ...
              return (
                <div
                  ...
                  onClick={() => setSelectedNationality(isSelected ? null : stat.id)}
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold" style={{ color: stat.color }}>
                        #{stats.nationalityStats.indexOf(stat) + 1}
                      </p>
                    </div>
                  </div>
          ...
          <div className="mt-4 p-4 rounded-xl border-2 border-dashed" style={{
            borderColor: stats.isDiversifie ? '#22c55e' : '#f59e0b',
            background: stats.isDiversifie ? 'rgba(34,197,94,0.05)' : 'rgba(245,158,11,0.05)'
          }}>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Globe className={`w-6 h-6 ${stats.isDiversifie ? 'text-green-500' : 'text-amber-500'}`} />
                <div>
                  <p className="font-semibold text-gray-800">
                    {stats.isDiversifie ? '✅ Base clientèle diversifiée' : '⚠️ Base clientèle concentrée'}
                  </p>
                  <p className="text-sm text-gray-600">
                    {stats.nationalitesCount} nationalités représentées
                    {stats.isDiversifie ? ' (Objectif atteint)' : ` - ${stats.topNationality.name} domine avec ${stats.topPercentage.toFixed(1)}%`}
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-4">
                <div className="text-center">
                  <span className="text-xs text-gray-500">Nationalité principale</span>
                  <p className="text-lg font-bold text-gray-800">
                    {stats.topNationality.flag} {stats.topNationality.name}
                  </p>
                </div>
                <div className="w-24 h-3 bg-gray-200 rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full transition-all duration-1000"
                    style={{
                      width: `${Math.min(stats.topPercentage, 100)}%`,
                      background: stats.topNationality.color
                    }}
                  />
                </div>
              </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.nationalityStats} layout="vertical" margin={{ left: 20 }}>
                  ...
                  <Bar dataKey="value" fill="#8884d8" radius={[0, 8, 8, 0]} barSize={40}>
                    {stats.nationalityStats.map((entry, index) => (
        ...
        <div className="w-full bg-white rounded-xl p-6 shadow-lg hover:shadow-xl transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
              ...
              Analyse Comparative par Nationalité
            </h2>
          </div>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-100">
            <div className="flex items-center gap-2 text-blue-600">
              ...
              <span className="font-semibold">Nationalité Principale</span>
            </div>
            <p className="text-2xl font-bold text-gray-800 mt-2">
              {stats.topNationality.flag} {stats.topNationality.name}
            </p>
            <p className="text-sm text-gray-600">
              {stats.topNationality.value.toLocaleString()} clients ({stats.topPercentage.toFixed(1)}%)
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-xl p-4 border border-purple-100">
            ...
            <p className="text-2xl font-bold text-gray-800 mt-2">
              {stats.nationalitesCount} pays
            </p>
            <p className="text-sm text-gray-600">
              {stats.isDiversifie ? 'Base diversifiée' : 'Base concentrée'}
            </p>
          </div>
          ...
          <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-4 border border-green-100">
            ...
            <p className="text-sm text-gray-600">
              Part de la nationalité principale
            </p>
          </div>
        </div>
        ...
        <div className="w-full bg-gradient-to-r from-gray-800 via-gray-900 to-gray-800 text-white rounded-xl p-8 text-center shadow-2xl">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-medium">Rapport Nationalités</p>
            ...
            <div className="flex gap-6 text-xs opacity-60">
              ...
              <span>🏳️ {stats.nationalitesCount} nationalités</span>
